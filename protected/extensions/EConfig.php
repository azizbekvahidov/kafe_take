...
class EConfig extends CApplicationComponent
{
	...
	public $configTableName = 'settings';
	...
	public function set($key, $value)
	{
		...
		if (false === is_array($this->_config) || false === array_key_exists($key, $this->_config))
		{
			...
			{
                    $dbCommand = $db->createCommand("INSERT INTO `{$this->configTableName}` (`setting_name`, `setting_value`) VALUES (:key, :value)");
		...
		{
			$dbCommand = $db->createCommand("UPDATE `{$this->configTableName}` SET `setting_value` = :value WHERE `setting_name` = :key LIMIT 1");
			$dbCommand->bindValue(':value', serialize($value), PDO::PARAM_STR);		
			$dbCommand->bindParam(':key', $key, PDO::PARAM_STR);
			$dbCommand->execute();
		}
	...
	private function _getConfig($db, $cache)
	{
		...
		if (false === $cache || false === ($this->_config = $cache->get(self::CACHE_KEY)))
		{
			...
			{
				$this->_config[$row['setting_name']] = $row['setting_value'];
			}
	...
	{
		$db->createCommand("CREATE TABLE IF NOT EXISTS `{$this->configTableName}` (`setting_name` VARCHAR(100) PRIMARY KEY, `setting_value` TEXT) COLLATE = utf8_bin")->execute();
	}
